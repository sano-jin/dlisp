...
\section{導入}
\label{sec:orga26e51b}

\subsection{背景}
\label{sec:org853655d}
...
\subsection{本稿の構成}
\label{sec:org19705ee}

todo


\section{提案手法}
\label{sec:org1539bc4}
...
\subsection{実装}
\label{sec:orgdab44c6}

\lstset{language=Caml,label=orgb9d8f77,caption={History node},captionpos=b,numbers=none}
\begin{lstlisting}
type history_node =
  | Main of
      (node ref * node) *
...
\lstset{language=Caml,label=orge58861a,caption={Update},captionpos=b,numbers=none}
\begin{lstlisting}
...
\section{例題}
\label{sec:org4d7a420}

\lstset{language=Lisp,label=orgf2a1719,caption={Append},captionpos=b,numbers=none}
\begin{lstlisting}
;; ++ は append
...
\section{課題}
\label{sec:org06367eb}

occur checking はまだ実装していないので

\lstset{language=Lisp,label=orgc280fe8,caption={Circle},captionpos=b,numbers=none}
\begin{lstlisting}
(let ((x '(1 2 3)))
  (let ((y (++ x x)))
    (begin
