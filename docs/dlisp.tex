...
\section{提案手法}
\label{sec:org75d3efd}

\subsection{概要}
\label{sec:org4a14c0f}
...
\subsection{実装}
\label{sec:org1731870}

\lstset{language=Caml,label=orgc2d12c5,caption={History node},captionpos=b,numbers=none}
\begin{lstlisting}
type history_node =
  | Main of
      (node ref * node) *
...
\lstset{language=Caml,label=org18b956c,caption={Update},captionpos=b,numbers=none}
\begin{lstlisting}
...
\section{例題}
\label{sec:org8dcd37d}



\lstset{language=Lisp,label=org4eff553,caption={Append},captionpos=b,numbers=none}
\begin{lstlisting}
;; ++ は append
...
\section{課題}
\label{sec:org15bcd34}

occur checking はまだ実装していないので

\lstset{language=Lisp,label=org0314d1d,caption={Circle},captionpos=b,numbers=none}
\begin{lstlisting}
(let ((x '(1 2 3)))
  (let ((y (++ x x)))
    (begin
