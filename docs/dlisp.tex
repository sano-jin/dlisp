...
\section{導入}
\label{sec:org901dab9}

\subsection{背景}
\label{sec:org66546d4}
...
\subsection{先行研究}
\label{sec:orga38fd24}

要調査

\subsubsection{可逆プログラミング}
\label{sec:org7eba6c1}
\begin{description}
...
\subsection{本稿の構成}
\label{sec:orge84f69d}

todo


\section{提案手法}
\label{sec:orgaedb3e7}

\subsection{概要}
\label{sec:orge2bc0a9}
...
\subsection{実装}
\label{sec:orgb049d9c}

\lstset{language=Caml,label=orge62b4a9,caption={History node},captionpos=b,numbers=none}
\begin{lstlisting}
type history_node =
  | Main of
      (node ref * node) *
...
\lstset{language=Caml,label=orgd9ceb1e,caption={Update},captionpos=b,numbers=none}
\begin{lstlisting}
...
\section{例題}
\label{sec:orgcfbd616}

\lstset{language=Lisp,label=org12e56ff,caption={Append},captionpos=b,numbers=none}
\begin{lstlisting}
;; ++ は append
...
\section{課題}
\label{sec:orgbf56280}

occur checking はまだ実装していないので

\lstset{language=Lisp,label=org4aac1f2,caption={Circle},captionpos=b,numbers=none}
\begin{lstlisting}
(let ((x '(1 2 3)))
  (let ((y (++ x x)))
    (begin
