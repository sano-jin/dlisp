% Created 2021-10-19 Tue 17:34
% Intended LaTeX compiler: pdflatex
\documentclass[presentation, xetex, 8pt]{beamer}
...
\label{sec:orgf1d2e91}

\begin{frame}[label={sec:orgcc55546}]{概要}
...
\begin{frame}[label={sec:org1aec0f5}]{概要}
純粋さを担保するために，
...
\begin{frame}[label={sec:org39fbeda}]{導入}
...
\section{提案手法}
\label{sec:org46b93a7}

\label{sec:proposal}

\begin{frame}[label={sec:org24e07f8}]{概要}
破壊的更新の履歴を木構造で管理する．
...
\begin{frame}[label={sec:org62231e2},fragile]{実装}
 \lstset{language=Caml,label=org3d57e69,caption={History node},captionpos=b,numbers=none}
\begin{lstlisting}
type history_node =
  | Main of
      (node ref * node) *
...
\lstset{language=Caml,label=org13d6308,caption={Update},captionpos=b,numbers=none}
\begin{lstlisting}
...
\section{例題}
\label{sec:org08ace7c}

\lstset{language=Lisp,label=orgc482f3d,caption={Append},captionpos=b,numbers=none}
\begin{lstlisting}
;; ++ は append
...
\section{課題}
\label{sec:org05147ff}

\begin{frame}[label={sec:org0225ece},fragile]{Occur checking}
 自分自身を含むオブジェクトを連結してしまうと，
循環してしまうため（素朴な手法では）評価が無限ループしてしまう．

\lstset{language=Lisp,label=org4c94ff1,caption={Circle},captionpos=b,numbers=none}
\begin{lstlisting}
(let ((x '(1 2 3)))
  (let ((y (++ x x)))
    (begin
...
\begin{frame}[label={sec:org49d0ce2}]{その他最適化手法}
\begin{itemize}
\item 現実装は，すごく安直で，
...
\section{関連研究}
\label{sec:org618fc6d}

\begin{frame}[label={sec:orgab68679}]{Multiversion concurrency control}
...
\begin{frame}[label={sec:org3215234}]{可逆プログラミング}
\begin{description}
...
\begin{frame}[label={sec:org6b94f3e}]{Haskell の DList など}
